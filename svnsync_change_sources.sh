...
# Change source URL
new_source_url="https://svn.domain.com/repo/"

echo ""
echo "##############################"
echo "### Changing source URLs"
echo "##############################"
...
do
    ...
    echo "###################################"
    echo "Old source:"
    svn pg --revprop -r0 svn:sync-from-url file:///"$repo"
    repo_name=$(echo "$repo" | rev | cut -f1 -d"/" | rev)
    echo "New source:"
    tmp_source="$new_source_url""$repo_name"
    echo "$tmp_source"
    svn propset --revprop -r 0 svn:sync-from-url "$tmp_source" file://"$repo"
done
